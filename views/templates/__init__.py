...
class TemplateView(View):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        try:
            value = self.request.META['HTTP_AUTHORIZATION'].split()[1]
    ...
    def post(self, request, login, slug):
        ...
        try:
            ...
            return HttpResponse('ERROR: template "%s" not exists' % template_name, status=500)
        except Exception as e:
            return HttpResponse('%s: %s' % (type(e), str(e)), status=500)
        while '\n\n\n#' in out:
            out = out.replace('\n\n\n#', '\n\n#')
        while '\n\n\n<' in out:
            out = out.replace('\n\n\n<', '\n\n<')
